    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="p-8 rounded-3xl bg-slate-900 border border-slate-800 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">Vehicle: Two-Wheeler • Active Deliveries: 1 • Rating: 4.9/5</p>
        </div>
      ...
      <div className="space-y-4">
        <h2 className="text-xl font-bold text-white flex items-center gap-2">
          <Navigation className="w-5 h-5 text-emerald-400" /> Interactive Route Navigation
        </h2>
      ...
      <div className="space-y-4">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {donations.map((item) => {
              const isAssignedToMe = item.status === 'Volunteer Assigned' || item.status === 'Picked Up';
                ...
                <div key={item.id} className="glass-card rounded-2xl p-6 border border-slate-800 space-y-4 flex flex-col justify-between">
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400">Donor: {item.restaurant_name}</p>
                    </div>
                    ...
                    <div className="p-4 rounded-xl bg-slate-950 border border-slate-800 space-y-3 text-xs">
                      <div className="flex items-start gap-2.5">
                        ...
                        <div>
                          ...
                          <p className="font-medium text-slate-200">{item.pickup_address}</p>
                        </div>
                      </div>
