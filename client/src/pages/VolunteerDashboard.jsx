...
import { Truck, CheckCircle2, Navigation, User, Phone, Bike, Edit3, Save, PackageCheck, ShieldCheck, FileText, UserPlus, Users, Image } from 'lucide-react';
  ...
  return (
    <PortalLayout>
      <div className="space-y-8">
        ...
        {/* Volunteer Driver Header with Driver Photo */}
        <div className="p-8 rounded-3xl bg-slate-900 border border-slate-800 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            ...
            >
              <UserPlus className="w-4 h-4" /> {isRegisteringDriver ? 'Close Form' : '+ Register Driver with Photo'}
            </button>
          </div>
        </div>

        {/* Register New Volunteer Driver Form with Photo Upload/Selection */}
        {isRegisteringDriver && (
          <div className="p-8 rounded-3xl bg-slate-900 border border-blue-500/40 space-y-6 shadow-2xl animate-in fade-in">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-bold text-white flex items-center gap-2">
                <UserPlus className="w-5 h-5 text-blue-400" /> Driver Registration & Photo Verification
              </h2>
              <span className="text-xs text-slate-400 font-semibold">{drivers.length} Registered Drivers</span>
            </div>
            ...
            <form onSubmit={handleRegisterDriver} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Aadhar Card Number *</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Driver Verification Photo URL *</label>
                <div className="flex gap-2">
                  <input
                    type="text"
                    required
                    placeholder="e.g. /driver_photo.png or photo URL"
                    value={regForm.photo_url}
                    onChange={(e) => setRegForm({ ...regForm, photo_url: e.target.value })}
                    className="flex-1 px-4 py-3 rounded-xl bg-slate-950 border border-slate-800 text-slate-200 text-xs focus:outline-none focus:border-blue-500"
                  />
                  <img
                    src={regForm.photo_url || '/driver_photo.png'}
                    alt="Preview"
              ...
              <div className="md:col-span-2 flex justify-end pt-2">
                ...
                >
                  <UserPlus className="w-4 h-4" /> Save & Register Verified Driver
                </button>
              </div>
            </form>
          </div>
                ...
                return (
                  <div key={item.id} className="glass-card rounded-2xl p-6 border border-slate-800 space-y-4 flex flex-col justify-between">
                    ...
                    <div className="space-y-3">
                      ...
                      {/* Courier Driver Details Card with Photo */}
                      <div className="p-3.5 rounded-xl bg-blue-500/10 border border-blue-500/20 text-xs flex items-center gap-3.5">
                        <img
                          src={driverObj.photo_url || '/driver_photo.png'}
                          alt={driverObj.name}
                          className="w-12 h-12 rounded-xl object-cover border border-blue-400/40 shrink-0"
                        />
